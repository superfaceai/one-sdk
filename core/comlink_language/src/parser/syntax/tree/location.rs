...
impl RawLocation {
    pub fn empty() -> Self {
        Self { line: 0, column: 0, char_index: 0 }
    }
    
    /// Computes raw location for the end of the given `slice`.
    pub fn compute_end(slice: &str) -> Self {
        let (lest_line_start, newline_count) = slice.char_indices().fold(
            (0, 0), |acc, (index, ch)| if ch == '\n' {
                (index + 1, acc.1 + 1)
            } else {
                acc
            }
        ...
        Self {
            line: newline_count,
            column: slice.len() - lest_line_start,
    ...
    pub fn compute_syntax_text_end(text: SyntaxText) -> Self {
        text.try_fold_chunks(
            Self::empty(),
            |acc, slice| -> Result<Self, std::convert::Infallible> { Ok(acc + Self::compute_end(slice)) }
        ).unwrap()
    }
}
impl Add<Self> for RawLocation {
    ...
    fn add(self, rhs: Self) -> Self::Output {
        Self {
            ...
            column: if rhs.line > 0 { rhs.column } else { self.column + rhs.column },
