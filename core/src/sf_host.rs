pub mod unstable {
	...
	pub fn http_get(url: &str, headers: &[[&str; 2]]) -> HttpHandle {
		...
		unsafe {
			__import_http_get(
				...
				headers_str.as_ptr() as i32, headers_str.len() as i32
			) as u32
		}
	}
	pub fn http_response_read(handle: HttpHandle, out: &mut [u8]) -> usize {
		unsafe {
			__import_http_response_read(handle as i32, out.as_mut_ptr() as i32, out.len() as i32) as usize
		}
	}
	...
	extern "C" {
		...
		fn __import_http_get(url_ptr: i32, url_len: i32, headers_ptr: i32, headers_len: i32) -> i32;
