...
pub mod unstable {
	...
	pub fn link_to<H: SfCoreUnstable>(linker: &mut Linker<H>, mut store: &mut Store<H>) -> anyhow::Result<()> {
		let abort = Func::wrap(&mut store, |mut caller: Caller<'_, H>| -> Result<(), Trap> {
			caller.host_data_mut().abort()
		});
		// TODO: maybe should be in env::abort?
		linker.define(MODULE_NAME, "abort", abort).context("Failed to define sf_unstable::abort")?;

		let test_me = Func::wrap(&mut store, |mut caller: Caller<'_, H>, param: i32| -> Result<i32, Trap> {
			caller.host_data_mut().test_me(param)
		});
		linker.define(MODULE_NAME, "test_me", test_me).context("Failed to define sf_unstable::test_me")?;
		
		let http_get = Func::wrap(&mut store, |
		...
		linker.define(MODULE_NAME, "http_get", http_get).context("Failed to define sf_unstable::http_get")?;

		let http_read_response = Func::wrap(&mut store, |
			mut caller: Caller<'_, H>,
			handle: i32,
		...
		linker.define(MODULE_NAME, "http_read_response", http_read_response).context("Failed to define sf_unstable::http_read_response")?;
