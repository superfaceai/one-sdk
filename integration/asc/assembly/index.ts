import { http_get_std, http_read_response_string_std } from "./sf_core_unstable";

import { print, HttpRequest } from './sf_std/unstable';

export function sf_entry_asc_example(param: i32): i32 {
  const url = `https://swapi.dev/api/people/${param}`;
  // const headers = new Map<string, string[]>();
  // headers.set('accept', ['application/json', 'application/xml']);
  // headers.set('foo', ['bar']);

  // const response = HttpRequest.fire(url, 'GET', headers, null).response();
  // print(`test ${response.status}`);
  
  const http = http_get_std(url, ['accept', 'application/json', 'foo', 'bar', 'accept', 'application/xml']);
  const response_body = http_read_response_string_std(http);

  return response_body.length;
}
